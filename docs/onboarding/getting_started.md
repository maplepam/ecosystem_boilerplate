...
3. **`emp_ai_auth`** may still declare **`emp_ai_core`** via Git — update **`ref`** in the auth repo’s **`pubspec.yaml`** on Bitbucket, then **`git fetch` / `git checkout`** in **`packages/emp_ai_auth`** and **`git add packages/emp_ai_auth`** ([emp_ai_auth_dependency.md](../integrations/emp_ai_auth_dependency.md)).
