...
*For first-time users: you do **not** need prior experience with Melos or this repo. Follow the sections in order.*

**What this doc gives you**

- How to get the code (**clone** vs **fork**) and connect **your** Git remote.
- **Prerequisites** (Flutter, Melos, Git access for platform + auth deps), **demo shell routes** after first run, and optional **Figma** `.fig` metadata (**§2** and **§8**).
...
Deeper topics: [README.md — integrations hub](../README.md#integrations-hub) — [navigation](../integrations/navigation.md) (routes, redirects, `go` / `push`), [environment](../integrations/environment.md), [auth](../integrations/auth.md), [dart_defines.md](../platform/dart_defines.md) (`FLAVOR` + toggles), [ci_cd.md](../platform/ci_cd.md) (Bitbucket / Bitrise / multi-flavor), [adopting_the_boilerplate.md](adopting_the_boilerplate.md) (productize fork: auth, samples, RBAC), [architecture.md](../engineering/architecture.md), [miniapps.md](../engineering/miniapps.md), **[host_structure.md](../engineering/host_structure.md)** (full folder map), **[miniapp_packages_and_extract.md](../engineering/miniapp_packages_and_extract.md)** (external package / submodule / WebView onboarding).

**First commands:** [§2 below](#gs-2). **FAQ:** [faq.md](faq.md). **Stuck?** [troubleshooting.md](../platform/troubleshooting.md).
...
| [§1b](#gs-1b) | If you need to **change the platform** (`ecosystem-platform`)                              |
...
This workspace is **one Git repository** for the **host app**, **mini-apps**, and **tooling**. **Platform** (`ecosystem-platform`), **`emp_ai_auth`**, and **`emp_ai_ds`** are **Git submodules** under **`packages/`**; the host app wires them with **`path:`** dependencies (see **[emp_ai_auth_dependency.md](../integrations/emp_ai_auth_dependency.md)**). See also **[repositories_overview.md](../engineering/repositories_overview.md)**.

**Clone with submodules.** Use **`git clone --recurse-submodules <url>`** or, after a plain clone, **`git submodule update --init --recursive`**. Then **`dart pub get`** and **`dart run melos bootstrap`**. Without initialized submodules, **`flutter pub get`** in the app cannot resolve **`path:`** deps. To hack platform or auth in a **separate** clone, you can still use **`pubspec_overrides.yaml`** — [faq.md](faq.md), [dependencies.md — Local platform development](../engineering/dependencies.md#local-platform-development).

**Preferred setup for most teams**

1. **One remote = your product** — Push this **boilerplate** tree (`melos.yaml`, `apps/*`, `packages/` submodule pointers, docs, tools) to **your** GitHub/GitLab/Azure DevOps repo. **Submodule pins** live in Git (**`git submodule status`**); bump with plain Git (see **[emp_ai_auth_dependency.md § Bumping submodule pins](../integrations/emp_ai_auth_dependency.md#bumping-submodule-pins)**).
...
3. **SSH** for **GitHub** (platform submodule) and **Bitbucket** (auth + DS submodules) must work on every machine and in CI — see [emp_ai_auth_dependency.md](../integrations/emp_ai_auth_dependency.md).

**Summary:** **One clone** of **ecosystem_boilerplate** for product work; **platform**, **auth**, and **DS** revisions are pinned by **submodule commits** in Git. Fork vs “clone + repoint origin” is mostly about **whether you contribute upstream** to the boilerplate repo.
...
| **Origin**   | *Your* default remote (usually *your* GitHub/GitLab repo).                                                    |
| **Fork**     | A copy of a repo *on the hosting site* (GitHub “Fork” button). It keeps a link to upstream for pull requests. |
...
1. **Disconnect** from the boilerplate remote and point **origin** at **your** empty repo:
...
1. Optional: add the original repo as **upstream** so you can pull template updates later:
...
### If you need to change the **platform** (`ecosystem-platform`)

Platform code (`**emp_ai_foundation`**, `**emp_ai_core`**, `**emp_ai_ds_***`, `**emp_ai_app_shell**`) lives in the **`packages/ecosystem-platform`** submodule (or a separate clone for heavy work). Do this instead:

1. **Develop** in **[ecosystem-platform](https://github.com/maplepam/ecosystem-platform)**: branch, implement, **`dart run melos bootstrap`**, **`flutter analyze`**, tests. Open a **PR** on the platform repo and merge when ready.
2. **Point the boilerplate at the new platform commit:** **`cd packages/ecosystem-platform`**, **`git fetch`**, **`git checkout <merge-sha>`**, return to repo root, **`git add packages/ecosystem-platform`**, **`flutter pub get`** in the app (or **`dart run melos bootstrap`**), commit. Details: **[emp_ai_auth_dependency.md](../integrations/emp_ai_auth_dependency.md#bumping-submodule-pins)**.
3. **`emp_ai_auth`** uses **`path:`** into **`packages/ecosystem-platform`** and **`packages/emp_ai_ds`** — after you move those submodule commits, run **`flutter pub get`** / **`melos bootstrap`**; to ship **auth source** changes, **`git fetch` / `git checkout`** in **`packages/emp_ai_auth`** and **`git add packages/emp_ai_auth`** ([emp_ai_auth_dependency.md](../integrations/emp_ai_auth_dependency.md)).
4. **While iterating** before merges land: **`pubspec_overrides.yaml`** → **`path:`** at a local platform clone — [dependencies.md — Local platform development](../engineering/dependencies.md#local-platform-development), [`pubspec_overrides.yaml.example`](../../apps/emp_ai_boilerplate_app/pubspec_overrides.yaml.example).

Governance and ownership: [repositories_overview.md §7](../engineering/repositories_overview.md#7-contributions).
...
- **Submodules:** initialize once (**`git submodule update --init --recursive`** or clone with **`--recurse-submodules`**). **`melos bootstrap`** does not fetch submodules for you.
- **SSH** for **GitHub** + **Bitbucket** when submodule URLs use **`git@…`** ([emp_ai_auth_dependency.md](../integrations/emp_ai_auth_dependency.md)).
...
| Step | Command                                         | Why                                                                                                                                                                          |
| ---- | ----------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1    | `git submodule update --init --recursive`        | Required on fresh clones (unless you used **`git clone --recurse-submodules`**). Populates **`packages/ecosystem-platform`**, **`packages/emp_ai_auth`**, **`packages/emp_ai_ds`**. |
| 2    | `dart pub get`                                  | Tooling scripts at the repo root need packages.                                                                                                                              |
| 3    | `dart run melos bootstrap`                      | Runs **`flutter pub get`** for **`apps/**`** (resolves **`path:`** deps into **`packages/`** — [emp_ai_auth_dependency.md](../integrations/emp_ai_auth_dependency.md)). |
| 4    | `dart run melos run generate:miniapps`          | Builds the mini-app route catalog from `miniapps_registry.yaml`. **Required** after every fresh clone.                                                                       |
| 5    | `cd apps/emp_ai_boilerplate_app && flutter run` | Runs the host app.                                                                                                                                                           |
...
**Canonical diagrams and code-location guide** — [host_structure.md](../engineering/host_structure.md). **Guided reading order (four pages)** — [engineering/README.md](../engineering/README.md).
...
- **Archive / TestFlight:** `flutter build ipa` (your org may add `**--export-options-plist`**, extra defines, **Xcode schemes**, or **per-flavor `--target`** entrypoints).
...
- **Step-by-step** (Bitbucket, Bitrise): [ci_cd.md](../platform/ci_cd.md).
...
- **[Flavor catalog](../../apps/emp_ai_boilerplate_app/lib/src/config/boilerplate_environment_catalog.dart)** — default when `**AUTH_CLIENT_ID`** is omitted (`FLAVOR` selects the row; mobile vs web client id and redirect follow `kIsWeb`).
...
### 6b. Super-app vs standalone vs embeddedif you need
...
| [emp_ai_auth_dependency.md](../integrations/emp_ai_auth_dependency.md)            | Submodules, `path:` deps, SSH                                                          |
