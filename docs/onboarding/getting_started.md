...
**Commands shortcut:** [first_day.md](first_day.md) → [§2 below](#gs-2). **FAQ:** [faq.md](faq.md). **Stuck?** [troubleshooting.md](../platform/troubleshooting.md).
...
This workspace is **one Git repository** for the **host app**, **mini-apps**, and **tooling**. **Platform** Dart packages (`emp_ai_core`, design system, app shell, …) live in **[ecosystem-platform](https://github.com/maplepam/ecosystem-platform)** and resolve via `**git:`** dependencies. `**emp_ai_auth`** resolves from **Bitbucket** (second Git URL). See **[repositories_overview.md](../engineering/repositories_overview.md)** and **[emp_ai_auth_dependency.md](../integrations/emp_ai_auth_dependency.md)**.

**One `git clone` is enough (normal flow).** Clone **only** this boilerplate repo (or your fork). You do **not** clone **ecosystem-platform** or **auth** as separate working trees for day-to-day work: `**dart run melos bootstrap`** runs `**flutter pub get`**, which **downloads** those packages from Git into Pub’s cache using the URLs in `**apps/emp_ai_boilerplate_app/pubspec.yaml`**. The repo’s root `**packages/`** directory stays empty unless you add optional product-only packages. To edit platform sources on disk, clone ecosystem-platform elsewhere and use `**pubspec_overrides.yaml**` — [faq.md](faq.md), [dependencies.md — Local platform development](../engineering/dependencies.md#local-platform-development).

**Preferred setup for most teams**

1. **One remote = your product** — Push this **boilerplate** tree (`melos.yaml`, `apps/*`, docs, tools) to **your** GitHub/GitLab/Azure DevOps repo. Pin **platform** and **auth** `**ref`s** in `**pubspec.yaml`** and `**[docs/meta/platform_bom.yaml](../meta/platform_bom.yaml)`**.
...
3. **SSH or HTTPS** for **GitHub** (platform) and **Bitbucket** (auth + transitive `**emp_ai_ds`**) must work on every machine and in CI — see [emp_ai_auth_dependency.md](../integrations/emp_ai_auth_dependency.md).

**Summary:** **One clone** of **ecosystem_boilerplate** for product work; **platform** and **auth** are **versioned separately** via Git refs (documented in the BOM). Fork vs “clone + repoint origin” is mostly about **whether you contribute upstream** to the boilerplate repo.
...
Platform code (`**emp_ai_foundation`**, `**emp_ai_core`**, `**emp_ai_ds_***`, `**emp_ai_app_shell**`) is **not** edited inside this repo. Do this instead:

1. **Clone** **[ecosystem-platform](https://github.com/maplepam/ecosystem-platform)** (separate folder — your org may use a fork URL).
2. **Develop** there: branch, implement, `**dart run melos bootstrap`**, `**flutter analyze`**, tests as your team requires. Open a **PR** on the platform repo and merge when ready.
3. **Point the host at the new code:** copy the **merge commit SHA** (or a **release tag**) and set the **same `ref:`** on **every** `emp_ai_*` Git dependency in `**apps/emp_ai_boilerplate_app/pubspec.yaml`**. Update `**[docs/meta/platform_bom.yaml](../meta/platform_bom.yaml)`** to match.
4. `**emp_ai_auth**` also depends on `**emp_ai_core**` from platform — update the `**emp_ai_core**` `ref` in the **auth** repo’s `pubspec.yaml` on the branch you pin (e.g. `ecosystem_boilerplate`), then run `**flutter pub upgrade emp_ai_auth`** in the host app and commit `**pubspec.lock`** ([emp_ai_auth_dependency.md](../integrations/emp_ai_auth_dependency.md)).
5. **While iterating** (before the platform PR lands): use `**apps/emp_ai_boilerplate_app/pubspec_overrides.yaml`** to `**path:`** at your local platform clone — [dependencies.md — Local platform development](../engineering/dependencies.md#local-platform-development), `[pubspec_overrides.yaml.example](../../apps/emp_ai_boilerplate_app/pubspec_overrides.yaml.example)`.
...
- **Git dependencies:** `**dart run melos bootstrap`** runs `**flutter pub get`** for `**apps/****` and needs SSH (or HTTPS) access to **GitHub** and **Bitbucket** ([emp_ai_auth_dependency.md](../integrations/emp_ai_auth_dependency.md)).
...
| 2    | `dart run melos bootstrap`                      | Runs `**flutter pub get`** for `**apps/**`** (resolves **ecosystem-platform** + **auth** Git deps — [emp_ai_auth_dependency.md](../integrations/emp_ai_auth_dependency.md)). |
...
| [emp_ai_auth_dependency.md](../integrations/emp_ai_auth_dependency.md)            | Auth + platform Git URLs, BOM, SSH                                                     |
